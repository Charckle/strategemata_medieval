...
## Strike is fast; raise-back is slow.

const HANDLE_LEN := 18.0
const RAISE_ANGLE := -0.85
const STRIKE_ANGLE := 0.55
## Fraction of the cycle spent on the downward strike (rest is the slow return).
const STRIKE_FRAC := 0.22
...
func _draw() -> void:
	# Pivot is (0,0) = base of the handle; hammer builds upward (-Y).
	...
	draw_rect(Rect2(6.0, -HANDLE_LEN - 7.0, 5.0, 9.0), Color(0.62, 0.62, 0.66, 0.95), true)
